...
def sort(feature):
    ...
    for i in range(150):
        if feature[i] < MIN + A and feature[i] >= MIN:
            ...
            feature[i] = 6
        elif feature[i] <= MAX and feature[i] >= 7 * A + MIN:
            feature[i] = 7
    return feature
...
model = Sequential()
model.add(DropoutNoScale(drop_in, input_shape=(12,), name='dropIn'))
...
np.save('middle_weight_100hid_3bit.npy', middle_weight)
output_weight = weight[2]
np.save('output_weight_100hid_3bit.npy', output_weight)
